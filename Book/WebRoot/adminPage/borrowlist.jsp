	        	...
	        	<c:forEach var="borr" items="${borrowList}">
	        		<c:if test="${borr.borrstate == '1'}">
	        			<tr>
							...
							<td><a href="<%=basePath%>BorrowUpdateServlet?borrid=${borr.borrid}">修改</a></td>
						</tr>
	        		</c:if>
	        		<c:if test="${borr.borrstate == '0'}">
	        			<tr>
							...
							<td>${borr.borrstate}</td>
							<td>
								<a href="<%=basePath%>BorrowUpdateServlet?borrid=${borr.borrid}">修改</a> 
								&nbsp;&nbsp;&nbsp; 
								<a href="<%=basePath%>ReturnBookServlet?borrid=${borr.borrid}">归还图书</a>
							</td>
						</tr>
	        		</c:if>
	        	</c:forEach>
